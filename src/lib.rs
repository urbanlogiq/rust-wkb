...
fn write_point<W: Write, T: Into<f64> + Float + std::fmt::Debug>(c: &Coordinate<T>, out: &mut W) {
...
fn write_many_points<W: Write, T: Into<f64> + Float + std::fmt::Debug>(
    mp: &[Coordinate<T>],
    mut out: &mut W,
) {
...
pub fn geom_to_wkb<T: Into<f64> + Float + std::fmt::Debug>(geom: &Geometry<T>) -> Vec<u8> {
...
pub fn write_geom_to_wkb<W: Write, T: Into<f64> + Float + std::fmt::Debug>(
    geom: &Geometry<T>,
    mut result: &mut W,
) {
    ...
    match geom {
        ...
        &Geometry::Rect(ref rect) => {
            let poly = Geometry::Polygon(rect.to_polygon());
            write_geom_to_wkb(&poly, result)
        }
        &Geometry::Triangle(ref tri) => {
            let poly = Geometry::Polygon(tri.to_polygon());
            write_geom_to_wkb(&poly, result)
        }
    }
}
...
mod tests {
    ...
    fn wkb_to_point() {
        ...
        assert_eq!(
            geom_to_wkb(&wkb_to_geom(&mut bytes.as_slice()).unwrap()),
            bytes
        );
    }
